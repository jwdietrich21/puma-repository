...
type
  ...
  public
    ...
    procedure ReplaceSegment(const aSegmentName, SetID: Str3;
  ...
  begin
    ...
    begin
      try
        ...
        Result    := EncodeDateTime(theYear, theMonth, theDay, theHour,
          theMinute, theSecond, 0);
      except
...
begin
  ...
  begin
    if (aString <> '') and (FMessage <> nil) and
      (pos(STANDARD_COMP_SEP, aString) = 0) and
      (pos(STANDARD_SUBC_SEP, aString) <> 0) then
...
begin
  if FMessage = nil then
    subCSep := STANDARD_SUBC_SEP
  else
    subCSep := FMessage.Delimiters.SubComponentSeparator;
  theSubcomponent := NewSubComponent;
  if pos(subCSep, SubComponentText) = 0 then
...
begin
  ...
  if FMessage <> nil then
  begin
    if (aString <> '') and (pos(FMessage.Delimiters.FieldSeparator, aString) = 0) and
  ...
  begin
    ...
    if (aString <> '') and (pos(STANDARD_FIELD_SEP, aString) = 0) and
      ((pos(STANDARD_COMP_SEP, aString) > 0) or
      (pos(STANDARD_SUBC_SEP, aString) > 0)) then
      {true fields only}
      AllocComponents(aString);
  end;
...
begin
  if FMessage = nil then
    compSep := STANDARD_COMP_SEP
  else
    compSep := FMessage.Delimiters.ComponentSeparator;
...
begin
  ...
  if (FMessage = nil) then
  begin
    if (pos(STANDARD_COMP_SEP, ComponentText) = 0) then
    ...
    begin
      ...
      begin
        singleComponentText :=
          NextSection(ComponentText, lastPos, STANDARD_COMP_SEP);
  ...
  begin
    if (pos(FMessage.Delimiters.ComponentSeparator, ComponentText) = 0) then
      theComponent.ParseMessageString(ComponentText)
    else
    begin
      lastPos := 0;
...
begin
  ...
  repeat
    if (currSegment <> nil) and (currSegment.SegmentName = aSegmentName) and
      ((SetID = '0') or (currSegment.FirstOccurrence.FirstField.
      nextSibling.contentString = SetID)) then
    begin
      found  := True;
      Result := currSegment;
    end
...
procedure THL7Message.ReplaceSegment(const aSegmentName, SetID: Str3; by: THL7Segment);
var
