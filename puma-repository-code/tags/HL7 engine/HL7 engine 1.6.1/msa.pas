...
procedure GetMSA(aSegment: THL7Segment; out MSARecord: tMSA);
...
procedure GetMSA(message: THL7Message; out AckCode: tID; out controlID: str20;
  out textMessage: str80; out exSeqNum: tNM; out delAckType: char; out ErrorCond: tCE);
  deprecated;
...
begin
  if (aSegment <> nil) and (aSegment.segmentType = 'MSA') then
  begin
    ...
    if nextOccurrence <> nil then
      ...
      begin
        ...
        AckCode     := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
        controlID   := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
        textMessage := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
        exSeqNum    := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
        delAckType  := char(0); // deprecated field
        ErrorCond   := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
        MessageWaitingNumber := aSegment.FirstOccurrence.GetNextFieldContent(nextField);
